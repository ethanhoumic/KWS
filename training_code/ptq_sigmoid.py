...
def compute_symmetric_scale_zp(x_min: float, x_max: float, n_bits: int = 8):
    """
    Symmetric per-tensor quantization（用於 sigmoid 前的 conv 輸出）。
    Maps [-abs_max, abs_max] → [-127, 127]（int8 表示，Z = 0）

    abs_max    = max(|x_min|, |x_max|)
    scale      = abs_max / 127
    zero_point = 0（固定，和 weight 對稱量化一致）
    """
    qmax_signed = 2 ** (n_bits - 1) - 1           # 127
    abs_max = max(abs(x_min), abs(x_max))
    abs_max = max(abs_max, 1e-8)
    scale      = abs_max / qmax_signed
    zero_point = 0                                 # int8 對稱，Z=0
...
def quantize_activation(x: torch.Tensor, scale: float,
    ...
    if symmetric:
        ...
        x_q = (x / scale + zero_point).round().clamp(qmin, qmax).to(torch.int8)
    else:
...
def ptq(model, calib_loader, device,
    ...
    layer_names = [
        'conv1',       # conv1 output（sigmoid 輸入）：對稱量化，可正可負
        'sigmoid1',    # sigmoid1 output：非對稱量化，輸出範圍 (0,1)
        'conv2',       # conv2 output（sigmoid 輸入）：對稱量化
        'sigmoid2',    # sigmoid2 output：非對稱量化
        'linear',      # linear output：非對稱量化
        'dnn',         # dnn output（sigmoid 輸入）：對稱量化
        'sigmoid3',    # sigmoid3 output：非對稱量化
        'classifier',  # final logits
    ]

    # sigmoid 前的 conv/dnn 輸出用對稱量化，其餘用非對稱
    ...
    for name, (mn, mx) in act_ranges.items():
        if name in SYMMETRIC_LAYERS:
            s, zp = compute_symmetric_scale_zp(mn, mx, n_bits)
            mode = 'symmetric'
        else:
            s, zp = compute_asymmetric_scale_zp(mn, mx, n_bits)
            mode = 'asymmetric'
        act_quant[name] = {'scale': s, 'zero_point': zp}
        print(f"  {name:15s} [{mode:11s}]: scale={s:.6f}  zero_point={zp}")
...
def fake_quant_forward(model, act_quant, weight_quant, data, n_bits=8):
    ...
    # 對稱量化層（int8，Z=0）和非對稱量化層（uint8，Z!=0）的 clamp 範圍不同
    SYMMETRIC_LAYERS = {'conv1', 'conv2', 'dnn'}
    ...
    def quant_deq_act(x, name):
        ...
        if name in SYMMETRIC_LAYERS:
            qmin, qmax = -(2 ** (n_bits - 1) - 1), 2 ** (n_bits - 1) - 1  # [-127, 127]
        else:
            qmin, qmax = 0, 2 ** n_bits - 1                                 # [0, 255]
    ...
    x = quant_deq_act(x, 'conv1')        # 對稱量化，sigmoid 輸入
    x = torch.sigmoid(x)
    x = quant_deq_act(x, 'sigmoid1')     # 非對稱量化，sigmoid 輸出
    ...
    x = quant_deq_act(x, 'conv2')        # 對稱量化，sigmoid 輸入
    x = torch.sigmoid(x)
    x = quant_deq_act(x, 'sigmoid2')     # 非對稱量化，sigmoid 輸出
    ...
    x = quant_deq_act(x, 'linear')       # 非對稱量化，無 sigmoid

    # dnn
    w = dequant_w('dnn')
    b = dequant_b('dnn', act_quant['linear']['scale'])
    x = nn.functional.linear(x, w, b)
    x = quant_deq_act(x, 'dnn')          # 對稱量化，sigmoid 輸入
    x = torch.sigmoid(x)
    x = quant_deq_act(x, 'sigmoid3')     # 非對稱量化，sigmoid 輸出
