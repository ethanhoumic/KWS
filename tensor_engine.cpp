...
#define SIGMOID_ENTRIES 16384
#define SIG_OFFSET (SIGMOID_ENTRIES / 2)

// ── 表格 ─────────────────────────────────────────────────────────────
uint16_t sig_table[SIGMOID_ENTRIES];   // index = frac part (0~16383)

// ── 讀取 sig_table.txt ────────────────────────────────────────────────
void load_sig_table(const char *path) {
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        int32_t  i;
        uint16_t q;
        if (sscanf(line, "%d %hu", &i, &q) == 2)
            sig_table[i + SIG_OFFSET] = q;
    }
    fclose(fp);
}
...
int32_t requantize(int32_t acc, uint32_t M0, int64_t n, int32_t K, bool before_sigmoid) {

    // step 1: acc * M0, Q32.32
    ...
    // step 4: rounding（before sigmoid: Q5.8; others: Q32.0)
    int64_t rounding;
    int32_t result;

    if (before_sigmoid) {  // round to Q5.8
        rounding = (with_K >> 23) & 1;
        result   = (with_K >> 24) + rounding;
        if (result > 4095) result = 4095;
        else if (result < -4096) result = -4096;
    }
    else {             // round to Q32.0
        rounding = (with_K >> 31) & 1;
        result   = (with_K >> 32) + rounding;
    }

    return result;
}

uint32_t sigmoid(int16_t r) { // r is Q5.18
    
    if (r < -8192) return 0xFFFFFFFF;
    else if (r > 8191) return 0;
    else return sig_table[r + SIG_OFFSET];
...
int main() {
    ...
    // 路徑（依你的 outdir 調整）
    const char *acc_path = "./acc_preact_sigmoid/conv1_acc_sigmoid.txt";
    const char *M0_1_path  = "./layer_outputs_sigmoid/conv1_M0.txt";
    const char *n_1_path   = "./layer_outputs_sigmoid/conv1_n.txt";
    const char *K_1_path   = "./layer_outputs_sigmoid/conv1_K.txt";

    const char *sig_path = "./dw_data/sig_table.txt";

    const char *M0_2_path = "./layer_outputs_sigmoid/sigmoid1_M_out.txt";
    const char *n_2_path  = "./layer_outputs_sigmoid/sigmoid1_n_out.txt";

    const char *out_path = "sigmoid1_out_c_model.txt";
    ...
    // before dw
    load_1d_uint32(M0_1_path, M0_1,    C_OUT);
    load_1d_int64 (n_1_path,  n_1_arr, C_OUT);
    load_1d_int32 (K_1_path,  K_1,     C_OUT);

    // dw
    load_sig_table(sig_path);

    // after dw
    load_1d_uint32(M0_2_path, M0_2, 2);
    load_1d_int64(n_2_path,  n_2_arr, 2);
    ...
    for (int c = 0; c < C_OUT; c++) {
        for (int hw = 0; hw < N_SPATIAL; hw++) {
            ...
            // step 1：requantize acc → q3（int32）
            int32_t q3 = requantize(acc[idx], M0_1[c], n_1_arr[c], K_1[c], true);

            int16_t r = (int16_t)q3;

            // step 2, 3: dw simulation

            uint32_t sig = sigmoid(-r); // Q0.32

            // step 4: requantize sigmoid output → q4（uint8）
            
            int32_t q4 = requantize(sig, M0_2[0], n_2_arr[0], zp_out, false);

            output[idx] = (uint8_t)q4;
        }
    }
...
// ======================================================================================
// ========================= deprecated functions for sigmoid ===========================
// ======================================================================================
